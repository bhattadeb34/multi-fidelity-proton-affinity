...
Usage:
    python screening/scripts/plotting/plot_si_candidates.py --iter 1
...
SCRIPT_DIR   = Path(__file__).resolve().parent  # screening/scripts/plotting/
SCREENING_DIR = SCRIPT_DIR.parent.parent       # screening/
PROJECT_DIR  = SCREENING_DIR.parent           # proton-affinity-paper/
EXECUTION_DIR = SCREENING_DIR / "scripts" / "execution"
...
CONFIG = json.loads(
    (SCREENING_DIR / "config" / "pipeline_config.json").read_text()
)
...
def load_priority_leads(path: Path) -> tuple[list[str], dict[str, str]]:
    if not path.exists():
    ...
    smiles = leads["smiles"].tolist()
    names = dict(zip(leads["smiles"], leads["lead_label"]))
    return smiles, names
...
def main(
    ...
    data_dir = PATHS.dft_validation_dir(
        iteration,
        explicit=dft_dir,
        required_files=("lead_selection_analysis.csv", "top5_leads.csv"),
    )
    top5_path = top5_csv or (data_dir / "top5_leads.csv")
    top5_smiles, top5_names = load_priority_leads(top5_path)
    plt.rcParams.update({
    ...
    # Load DFT results
    df = pd.read_csv(data_dir / "lead_selection_analysis.csv")
    best = (df.sort_values("pa_dft_kcalmol", ascending=False)
            .reset_index(drop=True))

    print(f"Loaded {len(best)} candidates")
    print(
        f"PA range: {best['pa_dft_kcalmol'].min():.1f} - "
        f"{best['pa_dft_kcalmol'].max():.1f} kcal/mol"
    )

    sa_scores = best.set_index("smiles")["sa_score"].to_dict()
    ...
    for idx, row in best.iterrows():
        ...
        smiles = row["smiles"]
        pa_dft = row["pa_dft_kcalmol"]
        in_window = PA_LOW <= pa_dft <= PA_HIGH
        ...
        # Mark candidates excluded by the documented stability review.
        if not bool(row["stability_eligible"]):
            ax.text(0.97, 0.97, "†",
    ...
    dagger_handle = Line2D(
        ...
        label="Excluded by stability review",
    )
    legend_elements = [
        ...
        mpatches.Patch(facecolor="white", edgecolor=COLOR_INWINDOW,
                       linewidth=2.5,
                       label=(
                           f"In target PA window "
                           f"({PA_LOW:g}–{PA_HIGH:g} kcal/mol)"
                       )),
    ...
    for idx, row in best.iterrows():
        ...
        smiles = row["smiles"]
        pa_dft = row["pa_dft_kcalmol"]
        in_window = "YES" if PA_LOW <= pa_dft <= PA_HIGH else "NO"
        mol_label = top5_names.get(smiles, "---")
