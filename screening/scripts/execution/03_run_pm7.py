...
Usage:
    ...
    python screening/scripts/03_run_pm7.py --iter 1 --resume
    python screening/scripts/03_run_pm7.py --iter 1 --resume --retry-failed
...
def main(
    ...
    if cand_df["smiles"].duplicated().any():
        raise ValueError("Candidate table contains duplicate SMILES")
    ...
    all_records = []
    failed      = []
    current_smiles = set(cand_df["smiles"])
    mol_id_by_smiles = {
        smiles: mol_id for mol_id, smiles in enumerate(cand_df["smiles"])
    }

    completed_smiles = set()
    if resume and out_path.exists():
        existing = pd.read_parquet(out_path)
        existing = existing[existing["smiles"].isin(current_smiles)].copy()
        existing["mol_id"] = existing["smiles"].map(mol_id_by_smiles)
        all_records = existing.to_dict("records")
        completed_smiles.update(existing["smiles"].unique())
        log.info(
            f"Resume: reused {len(existing):,} site records for "
            f"{existing['smiles'].nunique():,} successful molecules"
        )

    if resume and fail_path.exists():
        existing_failed = pd.read_csv(fail_path)
        existing_failed = existing_failed[
            existing_failed["smiles"].isin(current_smiles)
        ].copy()
        existing_failed["mol_id"] = existing_failed["smiles"].map(mol_id_by_smiles)
        if retry_failed:
            log.info(
                f"Resume: retrying {existing_failed['smiles'].nunique():,} "
                "previously failed molecules"
            )
        else:
            failed = existing_failed.to_dict("records")
            completed_smiles.update(existing_failed["smiles"].unique())
            log.info(
                f"Resume: retained {existing_failed['smiles'].nunique():,} "
                "previous failures"
            )

    pending = cand_df[~cand_df["smiles"].isin(completed_smiles)].copy()
    tasks = [
        (row.smiles, mol_id_by_smiles[row.smiles])
        for row in pending.itertuples(index=False)
    ]
    log.info(
        f"Starting PM7 on {len(tasks):,} pending molecules "
        f"({len(completed_smiles):,} reused) ..."
    )
    ...
    with ProcessPoolExecutor(
        ...
        futures = {
            executor.submit(_worker_fn, task): (task[1], task[0])
            for task in tasks
        }
    ...
    if all_records:
        ...
        site_keys = ["smiles", "site_idx", "protonated_smiles"]
        conflicts = raw_df.groupby(site_keys)["pa_pm7_kcalmol"].nunique() > 1
        if conflicts.any():
            raise ValueError("Conflicting PM7 values found for a repeated site key")
        df = (raw_df
              .drop_duplicates(site_keys)
        ...
        for smi, name in [("c1c[nH]cn1", "imidazole"),
    ...
    if failed:
        (pd.DataFrame(failed)
         .drop_duplicates(["smiles"], keep="last")
         .sort_values("mol_id")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Reuse existing successful and failed molecules; run only new candidates",
    )
    parser.add_argument(
        "--retry-failed", action="store_true",
        help="With --resume, retry molecules recorded in pm7_failed.csv",
    )
