"""Run charge-aware MOPAC calculations for the screening pipeline."""
...
class MOPACCalculator:
    ...
    def __init__(self, method="PM7", mopac_executable=None):
        self.method = method
        self.mopac_executable = (
            mopac_executable
            or os.environ.get("MOPAC_EXECUTABLE")
            or "mopac"
        )
    ...
    def _check_mopac(self):
        """Resolve MOPAC and fail before scheduling calculations if unavailable."""
        resolved = shutil.which(self.mopac_executable)
        if resolved is None:
            raise FileNotFoundError(
                "MOPAC executable not found. Put 'mopac' on PATH or set "
                "MOPAC_EXECUTABLE=/absolute/path/to/mopac."
            )
        self.mopac_executable = resolved
        try:
            subprocess.run(
                [self.mopac_executable],
                capture_output=True,
            ...
            # Some MOPAC builds wait for input when invoked without arguments;
            # successful path resolution is sufficient in that case.
            return False
    ...
    def run_mopac_calculation(self, input_file):
        ...
        try:
            result = subprocess.run(
                [self.mopac_executable, input_file],
                capture_output=True, 
                text=True, 
                timeout=300
            )
