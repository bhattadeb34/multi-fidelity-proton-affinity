...
# Thermal enthalpy of H+ at 298.15 K used throughout the DFT workflow.
H_PROTON_KJ   = 6.197
...
def main(iteration: int, dft_dir_override: str = None) -> None:
    ...
    if pareto_df["smiles"].duplicated().any():
        raise ValueError("Pareto input contains duplicate SMILES")
    ...
    if not all_records:
        raise ValueError(f"No converged DFT site records found in {dft_dir}")
    df = pd.DataFrame(all_records)
    df['smiles'] = df['mol_idx'].map(smiles_from_log)
    parsed_smiles = set(df["smiles"].dropna())
    expected_smiles = set(pareto_df["smiles"])
    if parsed_smiles != expected_smiles:
        raise ValueError(
            "DFT parse does not cover the Pareto set exactly: "
            f"missing={sorted(expected_smiles - parsed_smiles)}, "
            f"extra={sorted(parsed_smiles - expected_smiles)}"
        )
    df = df.merge(
        pareto_df[['smiles', 'pa_pred_kcalmol', 'uncertainty', 'sa_score']],
    ...
    try:
        ...
        top5_path = iter_dir / "top5_leads.csv"
        if top5_path.exists():
            ...
            if missing:
                raise ValueError(
                    f"Priority-lead table is missing columns: {sorted(missing)}"
                )
            TOP5_SMILES = top5_df["smiles"].tolist()
            TOP5_LABELS = top5_df["lead_label"].tolist()
        else:
            TOP5_SMILES = []
            TOP5_LABELS = []
            log.warning(
                f"No priority-lead table at {top5_path}; run "
                "12_select_dft_leads.py before producing the final labeled plot."
            )
        TOP5_MARKERS = ['D', 's', '^', 'P', 'X']

        lo = min(plot_df['pa_best_kcal'].min(), plot_df['pa_pred_kcalmol'].min()) - 4
        hi = max(plot_df['pa_best_kcal'].max(), plot_df['pa_pred_kcalmol'].max()) + 4
        lims = [lo, hi]
        ...
        cbar.set_label('RF ensemble dispersion (kcal mol$^{-1}$)', fontsize=26)
        ...
        fig.savefig(fig_dir / f"iter{iteration}_pa_parity_final.pdf", dpi=300, bbox_inches='tight')
