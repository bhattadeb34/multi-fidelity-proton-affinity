...
Objectives (all minimized after transformation):
  ...
  2. RF dispersion : standard deviation across tree predictions (lower is
                     preferred; this is not calibrated uncertainty)
...
Selection procedure:
  1. Filter: Grotthuss-capable accept/flag verdict + configured PA window
...
MORGAN_FP_SIZE = CONFIG["featurization"]["morgan_fp_size"]
PARETO_CONFIG  = CONFIG["pareto"]
...
def smiles_to_fp(smiles: str):
    """Return a standard fixed-width Morgan bit fingerprint."""
...
def max_tanimoto_to_set(fp, selected_fps: list) -> float:
    if not selected_fps:
        return 0.0
...
def main(iteration: int, n_select: int) -> None:
    ...
    # A flag denotes a plausible/borderline carrier or an otherwise capable
    # scaffold with a novelty concern. It is retained for Pareto/DFT review;
    # only definitive rejects are removed.
    mask = (
        (df["final_verdict"].isin(PARETO_CONFIG["eligible_verdicts"])) &
        (df["pa_pred_kcalmol"] >= PA_TARGET_LOW) &
        (df["pa_pred_kcalmol"] <= PA_TARGET_HIGH)
    )
    pool = df[mask].copy().reset_index(drop=True)
    log.info(f"Pool after accept/flag + PA filter: {len(pool):,} molecules")

    if len(pool) == 0:
        log.error("No candidates passed filters — check PA window and LLM verdicts")
        sys.exit(1)

    # Compute standard 1024-bit Morgan fingerprints for all pool molecules.
    log.info("Computing fingerprints for diversity ...")
    pool_fps = [smiles_to_fp(smi) for smi in pool["smiles"]]
    invalid_fp = [i for i, fp in enumerate(pool_fps) if fp is None]
    if invalid_fp:
        raise ValueError(
            f"Cannot compute diversity fingerprints for {len(invalid_fp)} candidates"
        )
    ...
    for round_idx in range(n_select):
        ...
        # Objective 2: RandomForest tree-to-tree ensemble dispersion.
        uncertainty = pool.iloc[remaining]["uncertainty"].values

        # Objective 3: SA score
        sa = pool.iloc[remaining]["sa_score"].fillna(
            PARETO_CONFIG["missing_sa_score"]
        ).values
        ...
        configured_weights = PARETO_CONFIG["objective_weights"]
        weights = np.array([
            configured_weights["pa_proximity"],
            configured_weights["uncertainty"],
            configured_weights["sa_score"],
            configured_weights["diversity"],
        ])
        if not np.isclose(weights.sum(), 1.0):
            raise ValueError(f"Pareto objective weights must sum to 1, got {weights.sum()}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n-select", type=int, default=CONFIG["pareto"]["n_select"],
        help="Number of candidates to select for DFT (default: pipeline config)")
