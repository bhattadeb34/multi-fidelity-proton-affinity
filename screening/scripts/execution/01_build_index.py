...
Writes to:
    ...
    data/screening/processed/zinc_fingerprints.npy   -- (N, 1024) uint8 Morgan count matrix
    data/screening/processed/zinc_fp_keys.npy        -- (1024,) selected sparse Morgan identifiers
...
def build_fp_matrix(
    ...
    The JSON stores arbitrary integer identifiers from sparse Morgan count
    fingerprints.
    ...
    original screening-retrieval pipeline represented the sparse counts.

    Returns:
        fp_matrix  -- (N, n_bits) uint8 array
        fp_keys    -- (n_bits,) int64 array of selected sparse identifiers
    """
...
def main(dry_run: bool = False) -> None:
    ...
    if sascorer is None:
        raise RuntimeError(
            "Synthetic-accessibility scoring is required for the reported "
            "screening filter; refusing to write sentinel scores."
        )
    ...
    report = {
        ...
        "retrieval_fingerprint": (
            "radius-2 sparse Morgan counts restricted to the 1024 most "
            "frequent identifiers in the filtered library"
        ),
        "faiss_coordinates": "stored latent_1, latent_2, latent_3 columns",
        "dry_run":           dry_run,
