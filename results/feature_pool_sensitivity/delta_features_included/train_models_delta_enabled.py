...
def select_features(
    ...
    # Stage 2: Correlation filter (vectorized, capped at top 1500 by target corr)
    if len(names) > 1:
        ...
        order = np.argsort(-target_corr)[:1500]
    ...
    if len(names) > 0:
        ...
        MIN_FEATURES = 20
        # Fallback: if 1-SE selects too few features, use the best alpha
        ...
        # Final fallback: if still too few, keep top by |coef| from best-alpha
        if selected_mask.sum() < MIN_FEATURES:
            top_k = min(MIN_FEATURES, len(names))
...
def main():
    ...
    parser.add_argument("--group-col", type=str, default=None,
                        help="Column for GroupKFold (e.g. record_id for molecule-level grouping)")
