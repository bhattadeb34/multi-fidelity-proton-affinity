...
def load_dft_pa() -> pd.DataFrame:
    ...
    for rec in dataset.values():
        ...
        if rec_id and smi and pa is not None:
            rows.append({"record_id": rec_id, "neutral_smiles": smi,
                         "dft_pa_kjmol_lab": float(pa)})
...
def augment_with_dft(df: pd.DataFrame, dft_df: pd.DataFrame,
                     ...
                     *, molecule_level: bool = False) -> pd.DataFrame:
    """One-to-one DFT descriptor join using the PA-defining site."""
...
def get_models(n_train: int) -> dict:
    """Use the production estimator factory for the DFT control."""
...
def main():
    ...
    df_nist = df_nist.merge(
        dft_pa_df[["neutral_smiles", "dft_pa_kjmol_lab"]],
        on="neutral_smiles", how="left", validate="one_to_one",
    )
    ...
    print("  Compare against:  results/nist1155/  (PM7+cheminformatics)")
