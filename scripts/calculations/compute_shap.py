...
compute_shap.py
===============
SHAP analysis for the best evaluated tree-ensemble model on each dataset.
...
from train_models import get_models as production_get_models
...
# Set minimum font size to 24 as requested
TICK_SIZE  = 24
LABEL_SIZE = 26
TITLE_SIZE = 28
SPINE_LW   = 1.8
...
def get_best_tree_model(cv_path):
    """Resolve the best evaluated RF/ExtraTrees model from CV artifacts."""
    cv = json.loads(cv_path.read_text())
    candidates = {
        name: cv["models"][name]
        for name in ("RandomForest", "ExtraTrees")
        if name in cv.get("models", {})
        and cv["models"][name].get("mae_pa_mean") is not None
    }
    if not candidates:
        raise ValueError(f"No supported tree model found in {cv_path}")
    name = min(candidates, key=lambda key: candidates[key]["mae_pa_mean"])
    return name
...
def plot_beeswarm(sv, X, names, title, out, top_n=TOP_N):
    ...
    # Increased figure size for large fonts
    fig, ax = plt.subplots(figsize=(14, max(10, top_n * 0.7)))
...
def plot_importance(sv, names, title, out, top_n=TOP_N):
    ...
    ax.set_title(title, fontsize=TITLE_SIZE, fontweight="bold", pad=20)
    ax.spines[["top","right"]].set_visible(False)
    ax.grid(axis="x", lw=1.0, alpha=0.4, ls="--")
...
def plot_dependence(sv, X, names, title, out, top_n=TOP_DEP):
    ...
    # Massively increased figure width: 8 inches per subplot
    fig, axes = plt.subplots(1, top_n, figsize=(8 * top_n, 9))
    fig.patch.set_facecolor("white")
    ...
    for ax, fi in zip(axes, idx):
        ...
        # Increased scatter point size to 80
        sc = ax.scatter(fv, sv_f, c=sv_f, cmap="coolwarm",
                        s=80, alpha=0.7, linewidths=0,
                        vmin=-vmax, vmax=vmax)
        ax.axhline(0, color="black", lw=1.5, ls="--", alpha=0.5)
        
        ax.set_xlabel(fname, fontsize=LABEL_SIZE, labelpad=15)
        # Newline applied to Y-axis
    ...
    fig.suptitle(title, fontsize=TITLE_SIZE+2, fontweight="bold", y=1.05)
    
    # Used w_pad to force spacing between columns
...
def plot_combined_beeswarm(sv_n, X_n, nm_n, sv_k, X_k, nm_k, out, top_n=TOP_N):
    ...
    # Massively increased figure width to accommodate font size 24
    fig, axes = plt.subplots(1, 2, figsize=(30, max(12, top_n*0.7)))
    fig.patch.set_facecolor("white")
...
def plot_paper_figure(sv_n, X_n, nm_n, sv_k, X_k, nm_k, out,
    ...
    ax.set_xlabel("SHAP value\n(kcal/mol)", fontsize=LABEL_SIZE, labelpad=15)
    ax.set_title("NIST — Molecular + PM7 features", fontsize=TITLE_SIZE,
    ...
    ax = plt.gca(); ax.set_facecolor("white")
    ax.set_xlabel("SHAP value\n(kcal/mol)", fontsize=LABEL_SIZE, labelpad=15)
    ax.set_title("k-means — Molecular + PM7 features", fontsize=TITLE_SIZE,
                 fontweight="bold", pad=15)
    ax.axvline(0, color="black", lw=1.5, ls="--", alpha=0.5)
    ax.spines[["top","right"]].set_visible(False)
    ...
    ax.set_xlabel("Mean |SHAP value|\n(kcal/mol)", fontsize=LABEL_SIZE, labelpad=15)
    ax.set_title("Feature importance — NIST", fontsize=TITLE_SIZE,
                 fontweight="bold", pad=15)
    ax.spines[["top","right"]].set_visible(False)
    ax.grid(axis="x", lw=1.0, alpha=0.4, ls="--")
    ...
    ax.set_xlabel("Mean |SHAP value|\n(kcal/mol)", fontsize=LABEL_SIZE, labelpad=15)
    ax.set_title("Feature importance — k-means", fontsize=TITLE_SIZE,
                 fontweight="bold", pad=15)
    ax.spines[["top","right"]].set_visible(False)
    ax.grid(axis="x", lw=1.0, alpha=0.4, ls="--")
    ax.text(-0.15, 1.05, labels[3], transform=ax.transAxes,
            fontsize=TITLE_SIZE+2, fontweight="bold")
    ax.tick_params(axis='both', labelsize=TICK_SIZE)

    # Applied wider padding to ensure Y labels don't get crunched
...
def run_shap(dataset):
    ...
    missing = [f for f in consensus if f not in df.columns]
    if missing:
        raise ValueError(
            f"{len(missing)} consensus features are absent from the final "
            f"training table; refusing a partial SHAP model: {missing[:10]}"
        )
    avail = list(consensus)
    log.info(f"  Consensus features: {len(avail)}")

    if not avail:
        raise ValueError("No >=3/5-fold consensus features were found")

    X_raw = df[avail].values.astype(np.float64)
    y     = df[target_col].values.astype(np.float64)
    X, _  = impute(X_raw)

    model_name = get_best_tree_model(results_sub / "cv_results.json")
    model = production_get_models(n_train=len(df))[model_name]
    log.info(
        "  Training %s (n_estimators=%s) on %s features ...",
        model_name,
    ...
    cv_path = results_sub / "cv_results.json"
    provenance = {
        "interpretation_scope": "descriptive SHAP for final full-data refit",
        "model": model_name,
        "n_samples": int(len(df)),
        "n_features": int(len(avail)),
        "consensus_rule": "selected in at least 3 of 5 outer CV folds",
        "target": target_col,
        "cv_results_sha256": hashlib.sha256(cv_path.read_bytes()).hexdigest(),
        "features": avail,
    }
    (shap_dir / "provenance.json").write_text(json.dumps(provenance, indent=2))
