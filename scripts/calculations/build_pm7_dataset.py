...
def main():
    ...
    if nist_file.exists():
        ...
        log.warning(f"Not found: {nist_file}  — expected at {nist_file.relative_to(SCRIPT_DIR.parent.parent)}")
    ...
    if kmeans_file.exists():
        ...
        log.warning(f"Not found: {kmeans_file}  — expected at {kmeans_file.relative_to(SCRIPT_DIR.parent.parent)}")
    ...
    log.info(f"Wrote {json_path.relative_to(SCRIPT_DIR.parent.parent)}  ({len(dataset)} records)")
