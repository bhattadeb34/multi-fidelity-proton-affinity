...
def parse_folder_molecule(mol_dir: Path) -> dict | None:
    ...
    return {
        ...
        "metadata": {
            "source":       "folder",
            "mol_dir":      str(mol_dir.relative_to(SCRIPT_DIR.parent)),
...
def parse_json_molecule(json_path: Path) -> dict | None:
    ...
    return {
        ...
        "metadata": {
            "source":        "json",
            "json_path":     str(json_path.relative_to(SCRIPT_DIR.parent)),
...
def main():
    ...
    log.info(f"Found {len(folder_dirs)} molecule folders in {FOLDER_ROOT.relative_to(SCRIPT_DIR.parent)}")
    ...
    log.info(f"Found {len(json_files)} JSON files in {JSON_ROOT.relative_to(SCRIPT_DIR.parent)}")
    ...
    log.info(f"Wrote {dataset_path.relative_to(SCRIPT_DIR.parent)}  ({len(dataset)} records)")
