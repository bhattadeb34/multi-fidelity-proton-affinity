...
def best_single_model(base_name: str) -> str:
    """Use the best no-DFT single-model class for a controlled comparison."""
    cv = json.loads((RESULTS_DIR / base_name / "cv_results.json").read_text())
    eligible = {
        name: metrics for name, metrics in cv["models"].items()
        if name not in {"PM7+bias", "VotingEnsemble"}
        and metrics.get("mae_pa_mean") is not None
    }
...
def load_paired(base_name: str, ref_col: str = "pa_true") -> pd.DataFrame:
    """Merge matched no-DFT and with-DFT predictions for one model class."""
    model = best_single_model(base_name)
    ...
    keys = ["fold", "sample_idx", "record_id"]
    merged = no.merge(
        dft[keys + ["pa_pred"]].rename(columns={"pa_pred": "pa_pred_dft"}),
        on=keys, how="inner", validate="one_to_one",
    )
    if len(merged) != len(no) or len(merged) != len(dft):
        raise ValueError(
            f"Unpaired predictions for {base_name}/{model}: "
            f"no-DFT={len(no)}, DFT={len(dft)}, paired={len(merged)}"
        )
...
def main():
    ...
    stats  = rf"k-means: mean $\Delta$ = {mean_d:+.2f} ± {std_d:.2f} kcal/mol"
