...
def get_best_model(dataset_name: str) -> str:
    """Return the best evaluated single model for this result set."""
...
def compute_overall_mae(preds: pd.DataFrame, model: str) -> tuple:
    ...
    df["pa_absolute_error"] = np.abs(df["pa_pred_kcal"] - df["pa_true_kcal"])
    fold_maes = df.groupby("fold")["pa_absolute_error"].mean().values
