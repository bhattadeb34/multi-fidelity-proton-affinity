...
def _setup_parity_ax(
    ax, x, y, color, xlabel, ylabel, n_label="molecules",
    reference_axis="x",
):
    ...
    if reference_axis not in {"x", "y"}:
        raise ValueError("reference_axis must be 'x' or 'y'")
    r2 = r2_score(x, y) if reference_axis == "x" else r2_score(y, x)
...
def plot_parity_pm7_vs_dft_kmeans(data):
    ...
    _setup_parity_ax(ax, x, y, COLOR_KM,
                     "PM7 PA\n(kcal/mol)",
                     "B3LYP/def2-TZVP PA\n(kcal/mol)",
                     n_label=f"sites ({n_mol} molecules)",
                     reference_axis="y")
...
def plot_complexity_bottcher(data):
    ...
    # NIST (blue): place the label just to the right of its mean line.
    x_nist = nist_mean + 0.03 * x_span
    ...
    ax.set_xlabel("Böttcher Molecular Complexity Score",
                  fontsize=LABEL_SIZE)
    ...
    # Place the legend below the axes so it does not cover either distribution.
...
def plot_parity_combined(data):
    ...
    _draw_panel(axes[2], xc, yc, COLOR_KM,
                ...
                override_n=n_mol_c, reference_axis="y")
