...
RESULTS_DIR = Path(os.environ.get("MFPA_RESULTS_DIR", PROJECT_DIR / "results"))
FIG_DIR     = Path(os.environ.get("MFPA_FIGURES_DIR", PROJECT_DIR / "figures"))
...
def get_ref_mae(cv_results_dir: Path) -> tuple[str, float]:
    """Return best single-model name and MAE from cv_results.json."""
    ...
    best = min(
        (m for m in cv["models"] if m not in {"PM7+bias", "VotingEnsemble"}),
...
if __name__ == "__main__":
    ...
    if run_nist:
        ...
        model_nist_pm7, ref_nist_pm7 = get_ref_mae(RESULTS_DIR / "nist1155")
        model_nist_dft, ref_nist_dft = get_ref_mae(RESULTS_DIR / "nist1155_dft")
        ...
        plot_single(lc_nist_pm7,
                    ...
                    ref_label=f"5-fold CV MAE: {ref_nist_pm7:.2f} kcal/mol ({model_nist_pm7})")
        plot_single(lc_nist_dft,
                    title="NIST — Molecular + PM7 + DFT features",
                    output_stem="learning_curve_nist_dft",
                    color="#2166AC",
                    ref_mae=ref_nist_dft,
                    ref_label=f"5-fold CV MAE: {ref_nist_dft:.2f} kcal/mol ({model_nist_dft})")
    ...
    if run_kmeans:
        ...
        model_km_pm7, ref_km_pm7 = get_ref_mae(RESULTS_DIR / "kmeans251")
        model_km_dft, ref_km_dft = get_ref_mae(RESULTS_DIR / "kmeans251_dft")
        ...
        plot_single(lc_km_pm7,
                    ...
                    color="#D01C8B",
                    ref_mae=ref_km_pm7,
                    ref_label=f"5-fold CV MAE: {ref_km_pm7:.2f} kcal/mol ({model_km_pm7})")
        plot_single(lc_km_dft,
                    title="k-means — Molecular + PM7 + DFT features",
                    output_stem="learning_curve_kmeans_dft",
                    color="#2166AC",
                    ref_mae=ref_km_dft,
                    ref_label=f"5-fold CV MAE: {ref_km_dft:.2f} kcal/mol ({model_km_dft})")
    ...
    if all(x is not None for x in [lc_nist_pm7, lc_nist_dft, lc_km_pm7, lc_km_dft]):
        ...
        plot_combined(
            ...
            model_nist_pm7=model_nist_pm7, model_nist_dft=model_nist_dft,
            model_km_pm7=model_km_pm7, model_km_dft=model_km_dft,
        )
