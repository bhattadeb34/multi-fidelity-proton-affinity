"""Plot the complete filtered ZINC space and k-means representatives."""
...
def unique_input(explicit: Path | None, root: Path, pattern: str) -> Path:
    """Resolve one input file and fail rather than guessing among matches."""
    ...
    if len(matches) != 1:
        raise FileNotFoundError(
            ...
            "Pass the input path explicitly."
        )
    return matches[0]
...
DFT_PATH = unique_input(args.representatives, DATA_DIR, "DFT_enhanced*.csv")
BG_PATH = unique_input(
    args.background,
    DATA_DIR / "screening" / "zinc_raw",
    "filtered_*.csv",
)
OUT_DIR = args.output_dir or Path(os.environ.get("MFPA_FIGURE_DIR", FIG_DIR))
...
build_report = DATA_DIR / "screening" / "processed" / "build_report.json"
if build_report.is_file():
    expected_rows = int(json.loads(build_report.read_text())["n_molecules"])
    if len(bg) != expected_rows:
        raise ValueError(
            f"Filtered-library row count differs from build_report.json: "
            f"{len(bg):,} versus {expected_rows:,}"
        )
bg_s = bg
...
def get_range(col):
    values = np.concatenate([
        bg_s[col].to_numpy(dtype=float),
        dft[col].to_numpy(dtype=float),
        dft['centroid_' + col].to_numpy(dtype=float),
    ])
