...
| `train_models.py`             | 5-fold CV over up to 15 regressors; automatic molecule-grouped k-means folds |
| `model_factory.py`            | Shared estimator definitions and hyperparameters for production and ablation runs |
| `train_models_dft.py`         | Same + B3LYP DFT features (Section 4 ablation) |
| `run_nist_pm7_descriptor_ablation.py` | NIST sensitivity with the 26 neutral/protonated PM7 property columns removed |
| `learning_curve.py`           | Learning-curve sweeps (NIST + k-means, PM7 / DFT) |
| `compute_shap.py`             | SHAP analysis for the configured tree model |
...
| `audit_release.py`            | Read-only source, credential, portability, syntax, and checksum audit |
