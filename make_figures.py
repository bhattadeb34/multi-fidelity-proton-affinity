...
All scripts assume:
  ...
  - ``data/`` contains the complete raw and processed scientific data (not in-repo;
...
STEPS: list[Step] = [
    ...
    Step(
        name="SI: paired DFT-descriptor effect",
        script=REPO_ROOT / "scripts" / "plotting" / "plot_diff_esi.py",
        group="main",
        produces="figures/model_performance/diff_esi.pdf",
    ),
    Step(
        name="prospective error vs training-set similarity",
        script=REPO_ROOT / "scripts" / "analysis" / "tanimoto_bias_analysis.py",
        group="main",
        produces="figures/tanimoto_bias_plot_final.pdf",
    ),
    Step(
    ...
    Step(
        ...
        group="screening",
        args=("--iter", "1"),
        produces="figures/screening/iter1_pareto.pdf",
    ),
    Step(
        ...
        group="screening",
        args=("--iter", "1"),
        produces="figures/screening/iter1_pa_parity_final.pdf",
    ),
    Step(
        ...
        group="screening",
        args=("--iter", "1"),
        produces="figures/screening/*.pdf",
    ),
    Step(
